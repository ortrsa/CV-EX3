...
def blendDemo():
    im1 = cv2.cvtColor(cv2.imread('sky.jpeg'), cv2.COLOR_BGR2RGB) / 255
    im2 = cv2.cvtColor(cv2.imread('f18.jpeg'), cv2.COLOR_BGR2RGB) / 255
    mask = cv2.cvtColor(cv2.imread('mask.jpg'), cv2.COLOR_BGR2RGB) / 255
...
def main():
    ...
    img_path = 'dog.jpeg'
    lkDemo(img_path)
    pyrGaussianDemo('pyr_bit.jpeg')
    pyrLaplacianDemo('pyr_bit.jpeg')
    blendDemo()
